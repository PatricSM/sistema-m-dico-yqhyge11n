...
export default function ClinicaFlowApp() {
  ...
  useEffect(() => {
    ...
    r.setProperty('--accent', a.accent); r.setProperty('--accent-hover', a.hover)
    r.setProperty('--accent-active', a.active); r.setProperty('--accent-weak', a.weak); r.setProperty('--accent-weak-2', a.weak2)
  }, [accent])
  useEffect(() => {
    const r = document.documentElement.style; const map = RADII[radius]
    Object.keys(map).forEach(k => r.setProperty(k, map[k]))
  }, [radius])
  useEffect(() => {
    const r = document.documentElement.style
    if (shadow === 'plana') { r.setProperty('--sh-xs', 'none'); r.setProperty('--sh-sm', 'none'); r.setProperty('--sh-card', 'none') }
    else { r.setProperty('--sh-xs', '0 1px 2px rgba(16,24,40,0.05)'); r.setProperty('--sh-sm', '0 1px 2px rgba(15,48,106,0.05), 0 2px 6px rgba(45,127,249,0.06)'); r.setProperty('--sh-card', '0 1px 2px rgba(16,24,40,0.04), 0 6px 16px rgba(15,48,106,0.06)') }
  }, [shadow])
  ...
  const closeTweaks = () => { setTwOpen(false); window.parent.postMessage({ type: '__edit_mode_dismissed' }, '*') }
  ...
  return (
    <div id="app">
      ...
      <main id="main">
        <Topbar route={route} />
        <div id="screen" style={isInbox ? { overflow: 'hidden', background: 'var(--surface)' } : {}}>
      ...
      <TweaksPanel open={twOpen} onClose={closeTweaks}
        accent={accent} setAccent={setAccent} radius={radius} setRadius={setRadius} shadow={shadow} setShadow={setShadow} />
    </div>
  )
}
